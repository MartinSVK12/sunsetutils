                ...
                switch (args[0]){
                    ...
                    case "stack":
                        if(args.length > 1){
                            switch (args[1]){
                                case "origin": {
                                    Block block = Block.blocksList[world.getBlockId(Minecraft.getMinecraft().objectMouseOver.blockX, Minecraft.getMinecraft().objectMouseOver.blockY, Minecraft.getMinecraft().objectMouseOver.blockZ)];
                                    Vec3i pos = new Vec3i(Minecraft.getMinecraft().objectMouseOver.blockX, Minecraft.getMinecraft().objectMouseOver.blockY, Minecraft.getMinecraft().objectMouseOver.blockZ);
                                    ...
                                    commandSender.sendMessage(String.format("Set origin to %s:%s at %s!",block.getBlockName(world.getBlockMetadata(pos.x,pos.y,pos.z)),meta,pos));
                                    return true;
                                }
                                case "remove":
                                    try {
                                ...
                                case "add":
                                    if(args.length == 2){
                                        ...
                                        stack.add(inst);
                                        commandSender.sendMessage(String.format("Added %s:%s at %s (offset %s) to stack!",block.getBlockName(world.getBlockMetadata(pos.x,pos.y,pos.z)),meta,pos,offset));
                                    }
                                    return true;
                                case "replace":
                                    if(args.length == 3){
                                        try {
                                            ...
                                            commandSender.sendMessage(String.format("Set block at index %s to %s:%s at %s (offset %s)!",args[2],block.getBlockName(world.getBlockMetadata(pos.x,pos.y,pos.z)),meta,pos,offset));
                                        } catch (IndexOutOfBoundsException e){
                                            e.printStackTrace();
                                        }
                                    }
